...
class Lxy_buildproModuleSite extends WeModuleSite {
	...
	public function doWebHouseadd () {
		...
		if(!empty($hsid))
		{
			$item = pdo_fetch("SELECT * FROM ".tablename($this->housetalbe)." WHERE hsid = :hsid and weid=:weid", array(':hsid' => $hsid,':weid'=>$_W['uniacid']));
			$arrPic=json_decode($item['pic'],true);
				
			//$piclists = unserialize($item['pic']);
		}
		
		$buildsublist = pdo_fetchall("SELECT sid,title FROM ".tablename($this->subtable)." WHERE hid = :hid and weid=:weid order by sort desc", array(':hid' => $hid,':weid'=>$_W['uniacid']));
	...
	public function doWebAlbumadd () {
		...
		if(!empty($id))
		{
			$item = pdo_fetch("SELECT * FROM ".tablename($this->albumtable)." WHERE id = :id and weid=:weid", array(':id' => $id,':weid'=>$_W['uniacid']));
	...
	public function doWebBilladd() {
		...
		if (!empty($hid)) {
			$item = pdo_fetch("SELECT * FROM ".tablename($this->billtable)." WHERE hid = :hid and weid=:weid ", array(':hid' => $hid,':weid'=>$_W['uniacid']));
		}
		else
		{
	...
	public function doWebDelbuild()
	{
		...
		$hid = intval($_GPC['hid']);
		load()->func('file');
		$item = pdo_fetch("SELECT * FROM ".tablename($this->headtable)." WHERE hid = :hid and weid=:weid" , array(':hid' => $hid,':weid'=>$_W['uniacid']));
		if (empty($item)) {
	...
	public function doWebDelsubbuild()
	{
		global $_GPC,$_W;
		$sid = intval($_GPC['sid']);
		$item = pdo_fetch("SELECT * FROM ".tablename($this->subtable)." WHERE sid = :sid and weid=:weid" , array(':sid' => $sid,':weid'=>$_W['uniacid']));
		if (empty($item)) {
	...
	public function doWebDelfell()
	{
		global $_GPC,$_W;
		$yid = intval($_GPC['yid']);
		$item = pdo_fetch("SELECT * FROM ".tablename($this->felltable)." WHERE yid = :yid and weid=:weid" , array(':yid' => $yid,':weid'=>$_W['uniacid']));
		if (empty($item)) {
	...
	public function doWebDelexpert()
	{
		global $_GPC,$_W;
		$id = intval($_GPC['id']);
		$item = pdo_fetch("SELECT * FROM ".tablename($this->experttable)." WHERE id = :id and weid=:weid" , array(':id' => $id,':weid'=>$_W['uniacid']));
		if (empty($item)) {
			message('抱歉，该专家点评不存在或是已经删除！', '', 'error');
		}
		if(!empty($item['thumb']))
	...
	public function doMobileBuildindex()
	{
		...
		{
				//$hyurl= !empty($member) ? create_url('mobile/channel', array('name' => 'home', 'weid' => $_W['uniacid'])) : create_url('mobile/module/card', array('name' => 'member', 'weid' => $_W['uniacid']));
				$hyurl=url('mc/bond/card', array('i'=>$weid), $noredirect = false);
		}
		else 
		{
			$hyurl=str_replace('{fromuser}', $fromuser, $set['hyurl']);
		}
		$shareurl=$_W['siteroot'].'app/'.$this->createMobileUrl('buildindex', array('hid' =>$hid,'from_user' => base64_encode(authcode($fromuser, 'ENCODE'))));
	...
	public function doMobileHuxing() {
		global $_GPC, $_W;
		$hid = $_GPC['hid'];
		$item = pdo_fetchall("SELECT * FROM ".tablename($this->housetalbe)." WHERE weid=:weid and hid = :hid order by sort desc", array(':weid'=>$_W['uniacid'],':hid' => $hid));
		$res=array();		
		foreach ($item as $itv)
		{			
			$nowsid=$itv['sid'];
	...
	public function doMobileHuxingpic()
	{
		...
		if($type=='get')
		{
			...
			foreach ($pics as $pic){
				...
				$picarr[] = array('img'=>$pic['src'],'width'=>$pic['w']*2,'height'=>$pic['h']*2,'name'=>$pic['txt']);
			}
			$data['pics'] = $picarr;
			$res= array();
	...
	public function doWebFullviewdel()
	{
		...
		$hsid = intval($_GPC['hsid']);
		$id=intval($_GPC['id']);
		$item = pdo_fetch("SELECT * FROM ".tablename($this->fullviewtable)." WHERE hsid = :hsid and weid=:weid and id=:id" , array(':hsid' => $hsid,':weid'=>$_W['uniacid'],'id'=>$id));
		if (empty($item)) {
			message('抱歉，该全景不存在或是已经删除！', '', 'error');
		}
