...
class Ewei_ktvModuleSite extends WeModuleSite {
	...
	public $_search_key = '__ktv2_search';
	...
	public $_ktv_level_config = array(15 => '豪华ktv', 14 => '高档ktv', 13 => '舒适ktv', 12 => '经济型ktv', );
	...
	public function getItemTiles() {
		global $_W;
		$urls = array(
			array('title' => "ktv首页", 'url' => $this->createMobileUrl('index')),
	...
	public function doMobileIndex()
	{
		...
		$set = $this->_set_info;
		$hid = $_GPC['hid'];
		$user_info = pdo_fetch("SELECT * FROM " . tablename('ktv2_member') . " WHERE from_user = :from_user AND weid = :weid limit 1", array(':from_user' => $from_user, ':weid' => $weid));
		//独立用户
		if ($set['user'] == 2) {
			if (empty($user_info['id'])) {
				...
				//用户已经存在，判断用户是否登录
				$check = check_ktv_user_login($this->_set_info);
				if ($check) {
			...
			if (empty($user_info['id'])) {
				...
				$member['isauto'] = 1;
				$member['status'] = 1;
				pdo_insert('ktv2_member', $member);
				$member['id'] = pdo_insertid();
				$member['user_set'] = $set['user'];
				//自动添加成功，将用户信息放入cookie
				ktv_set_userinfo(0, $member);
			} else {
				if ($user_info['status'] == 1) {
					$user_info['user_set'] = $set['user'];
					//用户已经存在，将用户信息放入cookie
					ktv_set_userinfo(1, $user_info);
				} else {
					//用户帐号被禁用
					$msg = "抱歉，你的帐号被禁用，请联系ktv解决。";
					if ($this->_set_info['is_unify'] == 1) {
						$msg .= "ktv电话：" . $this->_set_info['tel'] . "。";
					}
	...
	public function check_version()
	{
		...
		if ($this->_version == 0) {
			...
			$sql = "SELECT id FROM " . tablename('ktv2') . " WHERE weid = :weid AND status = :status" . $where;
	...
	public function doMobilelogin()
	{
		...
		if (checksubmit()) {
			...
			$member['weid'] = $weid;
			$record = ktv_member_single($member);
			...
			if (!empty($record)) {
				...
				if (empty($record['status'])) {
					die(json_encode(array("result" => 0, "error" => "登录失败，您的帐号被禁止登录，请联系ktv解决！")));
				}

				$record['user_set'] = $set['user'];

				//登录成功
				ktv_set_userinfo(0, $record);
	...
	public function doMobileOrder()
	{
		...
		$sql = 'SELECT `title`, `mail`, `phone`, `thumb`, `description` FROM ' . tablename('ktv2') . ' WHERE `id` = :id';
		$reply = pdo_fetch($sql, array(':id' => $hid));
		if (empty($reply)) {
			if ($is_submit) {
				die(json_encode(array("result" => 0, "error" => "ktv未找到!")));
			} else {
				message('ktv未找到, 请联系管理员!');
			}
		}
		...
		$sql = "SELECT *, $pricefield AS roomprice FROM " . tablename('ktv2_room') . " WHERE `id` = :id AND `ktvid` = :ktvid ";
		$room = pdo_fetch($sql, array(':id' => $id, ':ktvid' => $hid));
		if (empty($room)) {
			if ($is_submit) {
		...
		$sql = 'SELECT `id`, `roomdate`, `num`, `status` FROM ' . tablename('ktv2_room_price') . ' WHERE `roomid` = :roomid
		...
		$r_sql = 'SELECT `roomdate`, `num`, `status`, ' . $pricefield . ' AS `m_price` FROM ' . tablename('ktv2_room_price') .
			' WHERE `roomid` = :roomid AND `weid` = :weid AND `ktvid` = :ktvid AND `roomdate` >= :btime AND ' .
			' `roomdate` < :etime';
		$params = array(':roomid' => $id, ':weid' => $weid, ':ktvid' => $hid, ':btime' => $btime, ':etime' => $etime);
		...
		if ($is_submit) {
			...
			pdo_insert('ktv2_order', $insert);
			$order_id = pdo_insertid();

			//如果有接受订单的邮件,
			if (!empty($reply['mail'])) {
				$subject = "微信公共帐号 [" . $_W['account']['name'] . "] 微ktv订单提醒.";
				$body = "您后台有一个预定订单: <br/><br/>";
				$body .= "预定ktv: " . $reply['title'] . "<br/>";
	...
	public function doMobilekeyword()
	{
		...
		$business_sql = "SELECT id, title FROM " . tablename('ktv2_business') . " WHERE weid = '{$this->_weid}'";
		...
		$brand_sql = "SELECT id, title FROM " . tablename('ktv2_brand') . " WHERE weid = '{$this->_weid}' AND status = 1 ORDER BY displayorder DESC";
	...
	public function doMobiledetail()
	{
		...
		$reply = pdo_fetch("SELECT * FROM " . tablename('ktv2') . " WHERE id = :id ", array(':id' => $hid));
		...
		if ($ac == "getDate") {
			...
			$sql = "SELECT id, ktvid, id as roomid, title, breakfast, thumb, thumbs, oprice, " . $pricefield . " as m_price";
			$sql .= " FROM " .tablename('ktv2_room');
			$sql .= " WHERE 1 = 1";
			$sql .= " AND ktvid = :ktvid";
			$sql .= " AND weid = :weid";
			$sql .= " AND status = 1";
			...
			foreach($room_list as $key => $value) {
				...
				$r_sql = "SELECT roomdate, num, status, oprice, " . $pricefield . " as m_price FROM " . tablename('ktv2_room_price');
				$r_sql .= " WHERE 1 = 1";
				$r_sql .= " AND roomid = " . $value['roomid'];
				$r_sql .= " AND weid = :weid";
				$r_sql .= " AND ktvid = :ktvid";
...
//            $sql = "SELECT * FROM " . tablename('ktv2_room');
//            $where = " WHERE 1 = 1";
//            $where .= " AND ktvid = $hid";
//            $where .= " AND status = 1";
//
//            $sql .= $where;
//            $count_sql = "SELECT (id) FROM " . tablename('ktv2_room') . $where;
//
//            $sql .= " ORDER BY displayorder, sortid DESC";
			...
			foreach($rooms as &$r){
				$pricedays = pdo_fetchall("select $pricefield as price,roomdate from ew_ktv2_room_price where roomid={$r['id']} and roomdate>=$btime and roomdate<=$etime");
			...
			//$total = pdo_fetchcolumn("select count(*) from ew_ktv2_room r left join ew_ktv2_room_price p on r.id = p.roomid ".$where);
			$page_array = get_page_array($total, $pindex, $psize);
			$data = array();
			$data['result'] = 1;
	...
	public function doMobileroomdevice()
	{
		...
		if(empty($id) || empty($hid)) {
			...
			$sql .= " FROM " .tablename('ktv2_room');
			$sql .= " WHERE id = :id AND ktvid = :ktvid AND status = 1";
			$sql .= " LIMIT 1";
...
//            $pricedays = pdo_fetchall("select $pricefield as price,roomdate from ew_ktv2_room_price where roomid={$item['id']} and roomdate>=$btime and roomdate<=$etime");
//
//
//            //找出$day天的价格记录
//            $totalprice =  0 ;
...
//                $item['price'] = round( $totalprice/$day );
			...
			if ($this->_set_info['is_unify'] == 1) {
				...
				$sql = 'SELECT `phone` FROM ' . tablename('ktv2') . ' WHERE id = :id ';
	...
	public function doMobilelist()
	{
		...
		if ($ac == "getDate") {
			...
			$sql .= " FROM " .tablename('ktv2') ." AS h";
			$sql .= " right JOIN " .tablename('ktv2_room') ." AS r ON h.id = r.ktvid";
			$sql .= " WHERE 1 = 1";
			$sql .= " AND r.weid = :weid";
			...
			//循环房间列表
			foreach($room_list as $key => $value) {
				$r_sql = "SELECT count(id) as num, min(" . $pricefield . ") as m_price FROM " . tablename('ktv2_room_price') . " as p";
				$r_sql .= " WHERE 1 = 1";
				$r_sql .= " AND roomid = " . $value['roomid'];
				$r_sql .= " AND status = 1";
				$r_sql .= " AND roomdate >=" . $btime ." AND roomdate <" .$etime;
			...
			foreach($room_list as $key => $value) {
				...
				//取出ktv最低价放入数组中
				if (array_key_exists($ktvid, $ktv_list)) {
					$old_price = $ktv_list[$ktvid]['m_price'];
					if ($new_price < $old_price) {
						$ktv_list[$ktvid] = $value;
					}
				} else {
					$ktv_list[$ktvid] = $value;
				}
			...
			switch ($order_name)
			{
				case 0:
					//优先级
					$ktv_list = array_sort($ktv_list, 'displayorder', 1);
					break;
				case 1:
					if ($order_type == 1) {
						//价格降序
						$ktv_list = array_sort($ktv_list, 'm_price', 1);
					} else {
						//价格升序
						$ktv_list = array_sort($ktv_list, 'm_price', 0);
					}
					break;
			}

			$total = count($ktv_list);
			...
			if ($total <= $psize) {
				...
				// 需要分页
				if($pindex > 0) {
					$list_array = array_chunk($ktv_list, $psize);
					$list = $list_array[($pindex-1)];
					//print_r($list);exit;
				} else {
					$list = $ktv_list;
			...
			ob_start();
			include $this->template('ktv_crumb');
			$data['code'] = ob_get_contents();
			ob_clean();
	...
	public function doMobileorderlist()
	{
       ...
       // var_dump($memberid);die();
		...
		if ($ac == "getDate") {
			...
			$where = " FROM " .tablename('ktv2_order') ." AS o";
			$where .= " LEFT JOIN " .tablename('ktv2') ." AS h ON o.ktvid = h.id";
			$where .= " WHERE 1 = 1";
	...
	public function doMobileorderdetail()
	{
		...
		$sql .= " FROM " .tablename('ktv2_order') ." AS o";
		$sql .= " LEFT JOIN " .tablename('ktv2') ." AS h ON o.ktvid = h.id";
		$sql .= " WHERE 1 = 1";
		$sql .= " AND o.id = :id";
		...
		$params['module'] = "ewei_ktv";
		$params['ordersn'] = $item['ordersn'];
		$params['tid'] = $item['id'];
		$params['user'] = $_W['fans']['from_user'];
		$params['fee'] = $item['sum_price'];
		$params['title'] = $_W['account']['name'] . "ktv订单{$item['ordersn']}";
	...
	public function doMobileorderpay(){
		...
		$order = pdo_fetch("SELECT * FROM " . tablename('ktv2_order') . " WHERE id = :id", array(':id' => $orderid));
		...
		$params['title'] = $_W['account']['name'] . "ktv订单{$order['ordersn']}";
		$this->pay($params);
	...
	public function payResult($params) {
		global $_GPC, $_W;
		$weid = $this->_weid;
		$sql = 'SELECT * FROM ' . tablename('ktv2_order') . ' WHERE `id` = :id AND `weid` = :weid';
		$order = pdo_fetch($sql, array(':id' => $params['tid'], ':weid' => $weid));
		pdo_update('ktv2_order', array('paystatus' => 1), array('id' => $params['tid']));

		$sql = 'SELECT `email`, `mobile` FROM ' . tablename('ktv2_set') . ' WHERE `weid` = :weid';
		...
		if ($params['from'] == 'return') {
			$roomid = $order['roomid'];
			$room = pdo_fetch("SELECT score FROM " . tablename('ktv2_room') . " WHERE id = {$roomid} AND weid = {$weid} LIMIT 1");
			$score = intval($room['score']);
			if ($score) {
				$from_user = $this->_from_user;
				pdo_fetch("UPDATE " . tablename('ktv2_member') . " SET score = (score + " . $score . ") WHERE from_user = '" . $from_user . "' AND weid = " . $weid . "");
				if ($_W['member']['uid'] > 0) {
	...
	public function doMobileregister()
	{
		...
		if (checksubmit()) {
			...
			if (ktv_member_check(array('from_user' => $member['from_user'], 'weid' => $weid))) {
				die(json_encode(array("result" => 0, "error" => "非常抱歉，此用微信号已经被注册，你可以直接使用注册时的用户名登录，或者更换微信号注册！")));
			}

			if (ktv_member_check(array('username' => $member['username'], 'weid' => $weid))) {
			...
			$member['password'] = ktv_member_hash($member['password'], $member['salt']);
	...
	public function doWebktv() {
		global $_GPC, $_W;

		$op = $_GPC['op'];
		$weid = $_W['uniacid'];
		$ktv_level_config = $this->_ktv_level_config;
		load()->func('tpl');
		...
		if ($op == 'edit') {
			...
			if (checksubmit('submit')) {
				...
				if (empty($id)) {
					pdo_insert('ktv2', $insert);
				} else {
					pdo_update('ktv2', $insert, array('id' => $id));
				}
				message("ktv信息保存成功!", $this->createWebUrl('ktv'), "success");
			}
			$sql = 'SELECT * FROM ' . tablename('ktv2') . ' WHERE `id` = :id';
			...
			//品牌
			$sql = 'SELECT * FROM ' . tablename('ktv2_brand') . ' WHERE `weid` = :weid';
			$params = array(':weid' => $_W['uniacid']);
			$brands = pdo_fetchall($sql, $params);

			$sql = 'SELECT `title` FROM ' . tablename('ktv2_business') . ' WHERE `weid` = :weid AND `id` = :id';
			$params[':id'] = intval($item['businessid']);
			$item['ktvbusinesss'] = pdo_fetchcolumn($sql, $params);

			include $this->template('ktv_form');
		} else if ($op == 'delete') {

			$id = intval($_GPC['id']);

			if (!empty($id)) {
				$item = pdo_fetch("SELECT id FROM " . tablename('ktv2_order') . " WHERE ktvid = :ktvid LIMIT 1", array(':ktvid' => $id));
				if (!empty($item)) {
					message('抱歉，请先删除该ktv的订单,再删除该ktv！', '', 'error');
				}
			} else {
				message('抱歉，参数错误！', '', 'error');
			}

			pdo_delete("ktv2_order", array("ktvid" => $id));
			pdo_delete("ktv2_room", array("ktvid" => $id));
			pdo_delete("ktv2", array("id" => $id));

			message("ktv信息删除成功!", referer(), "success");
			...
			foreach ($_GPC['idArr'] as $k => $id) {
				...
				if (!empty($id)) {
					pdo_update('ktv2', array('status' => $show_status), array('id' => $id));
				}
			}
			...
			$temp = pdo_update('ktv2', array('status' => $_GPC['status']), array('id' => $id));
			if ($temp == false) {
			...
			$sql = 'SELECT id,title,description,thumb FROM ' . tablename('ktv2') . ' WHERE `weid`=:weid';
			$params = array();
			...
			$sql = 'SELECT COUNT(*) FROM ' . tablename('ktv2') . $where;
			...
			if ($total > 0) {
				...
				$sql = 'SELECT * FROM ' . tablename('ktv2') . $where . ' ORDER BY `displayorder` DESC LIMIT ' .
						($pindex - 1) * $psize . ',' . $psize;
				$list = pdo_fetchall($sql, $params);
				foreach ($list as &$row) {
					$row['level'] = $this->_ktv_level_config[$row['level']];
				}
	...
	public function doWebCopyroom() {
		...
		if (empty($ktvid) || empty($roomid)) {
			message('参数错误', 'refresh', 'error');
		}

		$item = pdo_fetch("SELECT * FROM " . tablename('ktv2_room') . " WHERE id = :id", array(':id' => $roomid));

		unset($item['id']);
		$item['status'] = 0;
		//$item['sortid'] = $roomid;

		pdo_insert('ktv2_room', $item);
		$id = pdo_insertid();
		$url = $this->createWebUrl('room', array('op' => 'edit', 'ktvid' => $ktvid, 'id' => $id));
		header("Location: $url");
		exit;
	}
	...
	public function doWebRoom_price() {
		...
		if ($ac == "getDate") {
			...
			$params = array();
			$sql = "SELECT r.* FROM " . tablename('ktv2_room') . "as r";
			$sql .= " WHERE 1 = 1";
			$sql .= " AND r.ktvid = $ktvid";
			$sql .= " AND r.weid = $weid";
			$list = pdo_fetchall($sql, $params);

			foreach ($list as $key => $value) {
				$sql = "SELECT * FROM " . tablename('ktv2_room_price');
				$sql .= " WHERE 1 = 1";
					...
					for ($i = 0; $i <= $pagesize; $i++) {
						...
						foreach ($item as $p_key => $p_value) {
							...
							if ($p_value['roomdate'] == $k) {
								...
								$list[$key]['price_list'][$k]['ktvid'] = $ktvid;
			...
			$ktvid = intval($_GPC['ktvid']);
			$roomid = intval($_GPC['roomid']);
			$price = $_GPC['price'];
			$pricetype = $_GPC['pricetype'];
			$date = $_GPC['date'];
			$roomprice = $this->getRoomPrice($ktvid, $roomid, $date);
			$roomprice[$pricetype] = $price;
			if (empty($roomprice['id'])) {
				pdo_insert("ktv2_room_price", $roomprice);
			} else {
				pdo_update("ktv2_room_price", $roomprice, array("id" => $roomprice['id']));
			}
			die(json_encode(array("result" => 1, "ktvid" => $ktvid, "roomid" => $roomid, "pricetype" => $pricetype, "price" => $price)));
			...
			$rooms = pdo_fetchall("select * from " . tablename("ktv2_room") . " where ktvid=" . $ktvid);
			...
			$start = $_GPC['start'];
			$end = $_GPC['end'];
			$list = pdo_fetchall("select * from " . tablename("ktv2_room") . " where id in (" . implode(",", $rooms) . ")");
			ob_start();
			...
			foreach ($rooms_arr as $v) {
				for ($time = $start; $time <= $end; $time+=86400) {
					...
					if (in_array($week, $days_arr)) {
						$roomprice = $this->getRoomPrice($ktvid, $v, date('Y-m-d', $time));
			...
			message("批量修改房价成功!", $this->createWebUrl('room_price', array("ktvid" => $ktvid)), "success");
		}
		$startime = time();
		$firstday = date('Y-m-01', time());
		//当月最后一天
	...
	public function doWebRoom_status() {
		...
		if ($ac == "getDate") {
			...
			message("批量修改房量房态成功!", $this->createWebUrl('room_status', array("ktvid" => $ktvid)), "success");
		}
	...
	//获取房型某天的记录
	private function getRoomPrice($ktvid, $roomid, $date) {
		global $_W;
		...
		if (empty($roomprice)) {
			$room = $this->getRoom($ktvid, $roomid);
			$roomprice = array(
				"weid" => $_W['uniacid'],
				"ktvid" => $ktvid,
				"roomid" => $roomid,
	...
	private function getRoom($ktvid, $roomid) {
		$sql = "SELECT * FROM " . tablename('ktv2_room');
		$sql .= " WHERE 1 = 1";
		$sql .= " AND ktvid = " . $ktvid;
		$sql .= " AND id = " . $roomid;
	...
	public function doWebRoom() {
		...
		if ($op == 'edit') {
			$id = intval($_GPC['id']);
			$ktvid = intval($_GPC['ktvid']);
			$ktv = pdo_fetch("select id,title from " . tablename('ktv2') . "where id=:id limit 1", array(":id" => $ktvid));
			if (!empty($id)) {
				$item = pdo_fetch("SELECT * FROM " . tablename('ktv2_room') . " WHERE id = :id", array(':id' => $id));
				if (empty($item)) {
			...
			if (checksubmit('submit')) {
				...
				if (empty($id)) {
					pdo_insert('ktv2_room', $data);
				} else {
					pdo_update('ktv2_room', $data, array('id' => $id));
				}
				pdo_query("update " . tablename('ktv2') . " set roomcount=(select count(*) from " . tablename('ktv2_room') . " where ktvid=:ktvid) where id=:ktvid", array(":ktvid" => $ktvid));
			...
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				$item = pdo_fetch("SELECT id FROM " . tablename('ktv2_order') . " WHERE roomid = :roomid LIMIT 1", array(':roomid' => $id));
				if (!empty($item)) {
			...
			pdo_delete('ktv2_room', array('id' => $id));
			pdo_delete('ktv2_order', array('roomid' => $id));
			pdo_query("update " . tablename('ktv2') . " set roomcount=(select count(*) from " . tablename('ktv2_room') . " where ktvid=:ktvid) where id=:ktvid", array(":ktvid" => $id));
			...
			foreach ($_GPC['idArr'] as $k => $id) {
				$id = intval($id);
				if (!empty($id)) {
					pdo_update('ktv2_room', array('status' => $show_status), array('id' => $id));
				}
			}
			$this->web_message('操作成功！', '', 0);
			exit();
			...
			$temp = pdo_update('ktv2_room', array('status' => $_GPC['status']), array('id' => $id));
			if ($temp == false) {
				message('抱歉，刚才操作数据失败！', '', 'error');
			} else {
				message('状态设置成功！', referer(), 'success');
			...
			$ktvid = intval($_GPC['ktvid']);
			$ktv = pdo_fetch("select title from " . tablename('ktv2') . "where id=:id limit 1", array(":id" => $ktvid));
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$sql = "";
			$params = array();
			...
			if (!empty($_GPC['ktvtitle'])) {
				$sql .= ' AND h.title LIKE :keywords';
				$params[':keywords'] = "%{$_GPC['ktvtitle']}%";
			}
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$list = pdo_fetchall("SELECT r.*,h.title as ktvtitle FROM " . tablename('ktv2_room') . " r left join " . tablename('ktv2') . " h on r.ktvid = h.id WHERE r.weid = '{$_W['uniacid']}' $sql ORDER BY h.id, r.displayorder, r.sortid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ktv2_room') . " r left join " . tablename('ktv2') . " h on r.ktvid = h.id WHERE r.weid = '{$_W['uniacid']}' $sql", $params);
			$pager = pagination($total, $pindex, $psize);
	...
	public function doWebOrder() {
       ...
       // checkauth();
		...
		$ktv = pdo_fetch("select id,title from " . tablename('ktv2') . " where id=:id limit 1", array(":id" => $ktvid));
		$roomid = intval($_GPC['roomid']);
		$room = pdo_fetch("select id,title from " . tablename('ktv2_room') . " where id=:id limit 1", array(":id" => $roomid));

		$op = $_GPC['op'];
		if ($op == 'edit') {
			$id = $_GPC['id'];
			if (!empty($id)) {
				$item = pdo_fetch("SELECT * FROM " . tablename('ktv2_order') . " WHERE id = :id", array(':id' => $id));
				if (empty($item)) {
			...
			if (checksubmit('submit')) {
				...
				$params = array();
				$sql = "SELECT id, roomdate, num FROM " . tablename('ktv2_room_price');
				$sql .= " WHERE 1 = 1";
				$sql .= " AND roomid = :roomid";
					...
					if ($room_date_list) {
						...
						foreach ($room_date_list as $key => $value) {
							...
							if ($num > 0) {
								...
								pdo_update('ktv2_room_price', array('num' => $now_num), array('id' => $value['id']));
							}
						}
					}
				}
				...
				pdo_update('ktv2_order', $data, array('id' => $id));
				message('订单信息处理完成！', $this->createWebUrl('order', array('ktvid' => $ktvid, "roomid" => $roomid)), 'success');
			}
			...
			$sql = "SELECT id, roomdate, num, status FROM " . tablename('ktv2_room_price');
			$sql .= " WHERE 1 = 1";
			$sql .= " AND roomid = :roomid";
			$sql .= " AND roomdate >= :btime AND roomdate < :etime";
			$sql .= " AND status = 1";
			...
			$member_info = pdo_fetch("SELECT from_user,isauto FROM " . tablename('ktv2_member') . " WHERE id = :id LIMIT 1", array(':id' => $item['memberid']));

			include $this->template('order_form');
		} else if ($op == 'delete') {
			$id = intval($_GPC['id']);
			$item = pdo_fetch("SELECT id FROM " . tablename('ktv2_order') . " WHERE id = :id LIMIT 1", array(':id' => $id));

			if (empty($item)) {
				message('抱歉，订单不存在或是已经删除！', '', 'error');
			}
			pdo_delete('ktv2_order', array('id' => $id));
			message('删除成功！', referer(), 'success');
		} elseif($op == 'deleteall') {
			foreach ($_GPC['idArr'] as $k => $id) {
				$id = intval($id);
			...
			$ktvtitle = $_GPC['ktvtitle'];
			$condition = '';
			$params = array();
			if (!empty($ktvtitle)) {
				$condition .= ' AND h.title LIKE :ktvtitle';
				$params[':ktvtitle'] = "%{$ktvtitle}%";
			}
			if (!empty($roomtitle)) {
			...
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$list = pdo_fetchall("SELECT o.*,h.title as ktvtitle,r.title as roomtitle FROM " . tablename('ktv2_order') . " o left join " . tablename('ktv2') .
				"h on o.ktvid=h.id left join " . tablename("ktv2_room") . " r on r.id = o.roomid  WHERE o.weid = '{$_W['uniacid']}' $condition ORDER BY o.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM  ' . tablename('ktv2_order') . " o left join " . tablename('ktv2') .
				"h on o.ktvid=h.id left join " . tablename("ktv2_room") . " r on r.id = o.roomid  WHERE o.weid = '{$_W['uniacid']}' $condition", $params);
	...
	public function doWebMember() {
		...
		if ($op == 'edit') {
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				$item = pdo_fetch("SELECT * FROM " . tablename('ktv2_member') . " WHERE id = :id", array(':id' => $id));
				if (empty($item)) {
			...
			if (checksubmit('submit')) {
				...
				if (!empty($_GPC['password'])) {
					$data['salt'] = random(8);
					$data['password'] = ktv_member_hash($_GPC['password'], $data['salt']);
					//$data['password'] = md5($_GPC['password']);
				}

				if (empty($id)) {
					$c = pdo_fetchcolumn("select count(*) from " . tablename('ktv2_member') . " where username=:username ", array(":username" => $data['username']));
					if ($c > 0) {
						message("用户名 " . $data['username'] . " 已经存在!", "", "error");
					}
					$data['createtime'] = time();
					pdo_insert('ktv2_member', $data);
				} else {
					pdo_update('ktv2_member', $data, array('id' => $id));
				}
			...
			$id = intval($_GPC['id']);
			pdo_delete('ktv2_member', array('id' => $id));
			pdo_delete('ktv2_order', array('memberid' => $id));
			message('删除成功！', referer(), 'success');
		} else if ($op == 'deleteall') {
			foreach ($_GPC['idArr'] as $k => $id) {
				$id = intval($id);
			...
			foreach ($_GPC['idArr'] as $k => $id) {
				$id = intval($id);
				if (!empty($id)) {
					pdo_update('ktv2_member', array('status' => $show_status), array('id' => $id));
				}
			}
			$this->web_message('操作成功！', '', 0);
			exit();
			...
			$temp = pdo_update('ktv2_member', array('status' => $_GPC['status']), array('id' => $id));
			...
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$list = pdo_fetchall("SELECT * FROM " . tablename('ktv2_member') . " WHERE weid = '{$_W['uniacid']}' $sql ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ktv2_member') . " WHERE weid = '{$_W['uniacid']}' $sql", $params);
			$pager = pagination($total, $pindex, $psize);
			include $this->template('member');
		}
	}

	public function doWebktvset() {
		global $_GPC, $_W;
		$id = intval($_GPC['id']);
		if (checksubmit('submit')) {
			$data = array(
			...
			if (!empty($id)) {
				pdo_update("ktv2_set", $data, array("id" => $id));
			} else {
				pdo_insert("ktv2_set", $data);
			}
			message("保存设置成功!", referer(), "success");
		}

		$sql = 'SELECT * FROM ' . tablename('ktv2_set') . ' WHERE `weid` = :weid';
		$set = pdo_fetch($sql, array(':weid' => $_W['uniacid']));
		if (empty($set)) {
			$set = array('user' => 1, 'reg' => 1, 'bind' => 1);
		}
		include $this->template("ktvset");
	}
	...
	public function doWebBrand() {
		...
		if ($op == 'edit') {
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				$item = pdo_fetch("SELECT * FROM " . tablename('ktv2_brand') . " WHERE id = :id", array(':id' => $id));
				if (empty($item)) {
			...
			if (checksubmit('submit')) {
				...
				if (empty($id)) {
					pdo_insert('ktv2_brand', $data);
				} else {
					pdo_update('ktv2_brand', $data, array('id' => $id));
				}
			...
			$id = intval($_GPC['id']);
			pdo_delete('ktv2_brand', array('id' => $id));
			message('删除成功！', referer(), 'success');
		} else if ($op == 'deleteall') {
			foreach ($_GPC['idArr'] as $k => $id) {
			...
			foreach ($_GPC['idArr'] as $k => $id) {
				...
				if (!empty($id)) {
					pdo_update('ktv2_brand', array('status' => $show_status), array('id' => $id));
				}
			}
			$this->web_message('操作成功！', '', 0);
			exit();
			...
			$temp = pdo_update('ktv2_brand', array('status' => $_GPC['status']), array('id' => $id));
			...
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$list = pdo_fetchall("SELECT * FROM " . tablename('ktv2_brand') . " WHERE weid = '{$_W['uniacid']}' $sql ORDER BY displayorder DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ktv2_brand') . " WHERE weid = '{$_W['uniacid']}' $sql", $params);
			$pager = pagination($total, $pindex, $psize);
	...
	public function doWebGetBusiness() {
		global $_W, $_GPC;
		$kwd = trim($_GPC['keyword']);
		$sql = 'SELECT * FROM ' . tablename('ktv2_business') . ' WHERE `weid`=:weid';
		$params = array();
		$params[':weid'] = $_W['uniacid'];
		if (!empty($kwd)) {
			$sql.=" AND `title` LIKE :title";
	...
	public function doWebBusiness() {
		...
		if ($op == 'edit') {
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				$item = pdo_fetch("SELECT * FROM " . tablename('ktv2_business') . " WHERE id = :id", array(':id' => $id));
				if (empty($item)) {
			...
			if (checksubmit('submit')) {
				...
				if (empty($id)) {
					pdo_insert('ktv2_business', $data);
				} else {
					pdo_update('ktv2_business', $data, array('id' => $id));
				}
			...
			$id = intval($_GPC['id']);
			pdo_delete('ktv2_business', array('id' => $id));
			message('删除成功！', referer(), 'success');
		} else if ($op == 'deleteall') {
			foreach ($_GPC['idArr'] as $k => $id) {
			...
			foreach ($_GPC['idArr'] as $k => $id) {
				...
				if (!empty($id)) {
					pdo_update('ktv2_business', array('status' => $show_status), array('id' => $id));
				}
			}
			$this->web_message('操作成功！', '', 0);
			exit();
			...
			$temp = pdo_update('ktv2_business', array('status' => $_GPC['status']), array('id' => $id));
			...
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$list = pdo_fetchall("SELECT * FROM " . tablename('ktv2_business') . " WHERE weid = '{$_W['uniacid']}' $sql ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ktv2_business') . " WHERE weid = '{$_W['uniacid']}' $sql", $params);
			$pager = pagination($total, $pindex, $psize);
