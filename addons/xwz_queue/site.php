    ...
    public function doMobileNumber() {
        ...
        if (!empty($typedata)) {
            ...
            if (empty($type)) {
                ...
                //后面的人
                $after_count = pdo_fetchcolumn('select count(*) from ' . tablename('xwz_queue_data') . ' where uniacid=:uniacid and rid=:rid and typeid=:typeid and status=0 and id>' . $typedata['id'], array(':uniacid' => $_W['uniacid'], ':rid' => $reply['rid'],':typeid'=>$typedata['typeid']));
...
//  
//         
//        $url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=xwz_queue&do=index&rid=' . $rid;
//        if($sendtype==1){
//                $data = json_encode(array(
//                    'keyword1' => array('value' => $type['tag'] . $data['number']),
//                    'keyword2' => array('value' => $type['title']),
//                    'keyword3' => array('value' => $number.'位'),
//                    'keyword4' => array('value' => $reply['heading']),
//                    'keyword5' => array('value' => date('Y-m-d H:i', $time))
//                ));
//                load()->classs('weixin.account');
//                $accObj = WeixinAccount::create($_W['uniacid']);
//                $access_token = $accObj->fetch_token();
...
//             $content.="取号时间：".date('Y-m-d H:i', $time)."\n\n";
...
//        }
    ...
    public function sendMessage($data,$notice =false) {
        ...
        if($sendtype==1){
                $data = json_encode(array(
                    ...
                    'keyword3' => array('value' => $number.'位'),
                    'keyword4' => array('value' => $reply['heading']),
                    'keyword5' => array('value' => date('Y-m-d H:i', $data['createtime']))
                ));
                load()->classs('weixin.account');
                $accObj = WeixinAccount::create($_W['uniacid']);
                $access_token = $accObj->fetch_token();
                if (!empty($access_token)) {
                    load()->func('communication');
                    $postarr = '{"touser":"' . $data['openid'] . '","template_id":"' . $template_id . '","url":"' . $url . '","data":' . $data . '}';
                    $res = ihttp_post('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=' . $access_token, $postarr);
                }
        }
        else if ($sendtype==2){
    ...
    public function doMobileLineups() {
        ...
        $datas = pdo_fetchall('select id,number,createtime from ' . tablename('xwz_queue_data') . ' where uniacid=:uniacid and rid=:rid and status=0 order by id asc limit 10', array(':uniacid' => $_W['uniacid'], ':rid' => $rid));
        foreach ($datas as &$d) {
