define("http://core.h5.lietou-static.com/v1/zepto/zepto.js", [], function(require, exports) {
    (function(undefined) {
    ...
    (function($) {
        ...
        $.ajax = function(options) {
            ...
            var reqHost = $("<a />").attr("href", settings.url)[0].host;
            if (!arguments[1] && reqHost && reqHost != location.host) {
                var iframe = $("<iframe />");
                iframe.attr({
                    src: "http://" + reqHost + "/ajaxproxy.html",
                    frameBorder: 0,
                    width: 0,
                    height: 0
                }).appendTo("body");
                iframe[0].onload = function() {
                    var oriSuccess = settings.success || function() {};
                    var oriError = settings.error || function() {};
                    var oriComplate = settings.complete || function() {};
                    settings.success = function() {
                        oriSuccess.apply(this, arguments);
                        iframe.remove();
                    };
                    settings.error = function() {
                        oriError.apply(this, arguments);
                        iframe.remove();
                    };
                    settings.complate = function() {
                        oriComplate.apply(this, arguments);
                        iframe.remove();
                    };
                    $.ajax(settings, this.contentWindow);
                };
    ...
    (function($) {
        ...
        zepto.qsa = function(node, selector) {
            return process(selector, function(sel, filter, arg) {
                try {
                    var taggedParent;
                    if (!sel && filter) sel = "*"; else if (childRe.test(sel)) taggedParent = $(node).addClass(classTag), 
                    sel = "." + classTag + " " + sel;
        ...
        $(document).ready(function() {
            ...
            $(document.body).bind("touchstart", function(e) {
                ...
                if (touch.x2 && Math.abs(touch.x1 - touch.x2) > 30 || touch.y2 && Math.abs(touch.y1 - touch.y2) > 30) swipeTimeout = setTimeout(function() {
                    ...
                    touch = {};
                }, 0); else if ("last" in touch) tapTimeout = setTimeout(function() {
                    var event = $.Event("tap");
    ...
    Zepto.ajaxSettings.complete = function(jqXHR, status) {
        var loginUrl = "";
        if (jqXHR) {
            loginUrl = jqXHR.getResponseHeader("LP_LOGIN_FORWARD") || "";
            if (loginUrl) {
