...
(function(a, d) {
	function b() {
		...
		if (SCREEN_SHOW_ALL) k / b > W / H ? k = W * b / H : b = H * k / W, c.style.marginTop = 0;
		else {
			var d = W * b / H;
	...
	window.onload = function() {
		...
		h.audio && (IS_ANDROID ? a.queue.loadFile({
			id: "sound",
			src: "../addons/weiyun_shuqian/template/audio/all.mp3"
		...
		h.noshare || a.queue.loadManifest({
            manifest: [{
                src: "../addons/weiyun_shuqian/template/img/share_tip.png",
                id: "share_tip"
            }]
        }, !1);
		h.followed || a.queue.loadManifest({
			manifest: [{
				src: "../addons/weiyun_shuqian/template/img/follow_anim.png",
				id: "follow"
			}]
		}, !1);
		//c.forQueue(a.queue);
	...
	a.showMoreGames = function(b) {
		if (null == e) {
			...
			var d = new createjs.Shape,
				f = e.getBounds();
			d.graphics.beginFill("#000").rect(0,
				0, -f.width, -f.height);
			e.hitArea = d;
...
function MyProgressBar(a, d, b) {
	...
	b.textAlign = "center";
	this.addChild(b);
	b = new createjs.Text("", "38px Arial", "black");
	b.x = a / 2;
	b.y = 260;
	b.textAlign = "center";
	this.addChild(b);
	b = new createjs.Text("", "32px Arial", "black");
	b.x = a / 2;
	b.y = d - 100;
	b.textAlign = "center";
	this.addChild(b)
