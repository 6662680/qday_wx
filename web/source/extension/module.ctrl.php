...
if($do == 'installed') {
	...
	if (!empty($modules)) {
		foreach ($modules as $mid => $module) {
			...
			$modules[$mid]['official'] = empty($module['issystem']) && (strexists($module['author'], 'WeEngine Team') || strexists($module['author'], '情天团队'));
...
if($do == 'permission') {
	...
	if (empty($module)) {
		message('你访问的模块不存在. 或许你愿意去情天云服务平台看看. ', 'http://addons.qdaygroup.com/web/index.php?keyword=' . $_GPC['title']);
	}
...
<manifest xmlns="http://www.qdaygroup.com" versionCode="{$versions}">
